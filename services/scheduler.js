...
async function cleanupOldStatusRows() {
  await db.run(
    "DELETE FROM monitors_status WHERE checked_at < datetime('now', ?)",
...
async function checkMonitor(monitor) {
  ...
  try {
    await db.run(
      'INSERT INTO monitors_status (slug, status, response_time, status_code) VALUES (?, ?, ?, ?)',
      [monitor.slug, currentStatus, responseTime, statusCode],
    );
