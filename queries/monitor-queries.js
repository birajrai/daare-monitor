...
const adminMonitors = `SELECT m.name, m.slug, m.url, m.interval,
...
const monitorBySlug = `SELECT m.name, m.slug, m.url, m.interval AS update_interval, s.last_checked AS last_checked_at
FROM monitors m
LEFT JOIN monitors_state s ON s.slug = m.slug
WHERE m.slug = ?`;
